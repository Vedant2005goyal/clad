...
template <bool Enabled> struct MutexStorage {};
template <> struct MutexStorage<true> {
#ifndef __CUDACC__
  mutable std::mutex m_TapeMutex;
  std::mutex& mutex() const { return m_TapeMutex; }
#endif
};
template <> struct MutexStorage<false> {
#ifndef __CUDACC__
  void mutex() const {}
#endif
};
...
template <typename T, std::size_t SBO_SIZE = 64, std::size_t SLAB_SIZE = 1024,
          bool is_multithread = false, bool DiskOffload = false>
class tape_impl : public detail::MutexStorage<is_multithread> {
  ...
  struct DiskStorage {
    ...
    CUDA_HOST_DEVICE DiskStorage()
        : data_ptr(nullptr), is_on_disk(false), disk_offset(0) {
      allocate();
    }
...
private:
  ...
  struct DiskInfo {
    ...
    std::size_t m_ActiveSlabs;
    std::size_t m_MaxRamSlabs;
    DiskInfo() : m_ActiveSlabs(0), m_MaxRamSlabs(1000) {}
  };
  struct Empty {};

  using DiskInfoType =
      typename std::conditional<DiskOffload, DiskInfo, Empty>::type;

  struct InternalState : public DiskInfoType {
  } m_state;
  ...
  DiskInfo& getDiskInfo() { return *reinterpret_cast<DiskInfo*>(&m_state); }
...
public:
  ...
  CUDA_HOST_DEVICE tape_impl()
      : m_head(nullptr), m_tail(nullptr), m_size(0), m_capacity(SBO_SIZE) {}
  ...
  CUDA_HOST_DEVICE reference back() {
    assert(m_size);
    if (m_size <= SBO_SIZE)
      return *(sbo_elements() + (m_size - 1));
    ensure_loaded(m_tail);
    std::size_t offset = (m_size - 1 - SBO_SIZE) % SLAB_SIZE;
    return *(m_tail->elements() + offset);
  }
